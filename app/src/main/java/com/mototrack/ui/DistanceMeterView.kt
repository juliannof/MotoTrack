...
class DistanceMeterView @JvmOverloads constructor(
    ...
    private val colorOn = Color.parseColor("#00BCD4")
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        setMeasuredDimension(
            ...
            resolveSize((24 * density).toInt(), heightMeasureSpec)
        )
    }
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        val gap = 3 * density
        // Nº de LEDs según el ancho disponible: ~12 dp por LED
        val leds = ((w + gap) / (12 * density)).toInt().coerceIn(6, 40)
        ...
        // El último LED es la ruta más larga: si no se ha llegado, va contorneado
        if (litCount < leds) {
